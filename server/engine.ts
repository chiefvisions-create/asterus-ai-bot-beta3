...
// ============== ADAPTIVE AI BRAIN INTERFACES ==============

interface MarketRegimeState {
  regime: 'trending_bullish' | 'trending_bearish' | 'ranging' | 'volatile' | 'accumulation' | 'distribution';
  confidence: number;
...
interface MetaLearningState {
  ...
  learningRate: number;
  adaptationSpeed: 'slow' | 'moderate' | 'fast' | 'turbo';
...
export class TradingEngine {
  ...
  // ============== ADAPTIVE AI BRAIN STATE ==============
  private aiBrainStates: Map<number, AIBrainState> = new Map(); // Per-bot AI brain
  private patternDatabase: PatternMemory[] = []; // Shared pattern memory across all bots
  private volatilityHistory: number[] = []; // Rolling volatility for percentile calculation
  private readonly MAX_VOLATILITY_HISTORY = 500;
  ...
  // Initialize pattern database with known profitable setups
  private initializePatternDatabase() {
    this.patternDatabase = [
      {
        patternId: 'oversold_bounce',
        conditions: { rsiRange: [15, 30], trendDirection: 'down', volatilityLevel: 'medium', regime: 'ranging', volumeProfile: 'surge' },
        outcome: { winRate: 0.65, avgReturn: 2.5, occurrences: 0, lastSeen: new Date() }
      },
      {
        patternId: 'trend_continuation',
        conditions: { rsiRange: [45, 65], trendDirection: 'up', volatilityLevel: 'low', regime: 'trending_bullish', volumeProfile: 'normal' },
        outcome: { winRate: 0.60, avgReturn: 1.8, occurrences: 0, lastSeen: new Date() }
      },
      {
        patternId: 'breakout_momentum',
        conditions: { rsiRange: [55, 75], trendDirection: 'up', volatilityLevel: 'high', regime: 'volatile', volumeProfile: 'surge' },
        outcome: { winRate: 0.55, avgReturn: 3.2, occurrences: 0, lastSeen: new Date() }
      },
      {
        patternId: 'mean_reversion_extreme',
        conditions: { rsiRange: [10, 25], trendDirection: 'neutral', volatilityLevel: 'high', regime: 'volatile', volumeProfile: 'normal' },
        outcome: { winRate: 0.58, avgReturn: 2.0, occurrences: 0, lastSeen: new Date() }
      },
      {
        patternId: 'accumulation_entry',
        conditions: { rsiRange: [30, 45], trendDirection: 'neutral', volatilityLevel: 'low', regime: 'accumulation', volumeProfile: 'declining' },
        outcome: { winRate: 0.62, avgReturn: 2.8, occurrences: 0, lastSeen: new Date() }
      }
    ];
  }
  
  // Get or initialize AI Brain state for a bot
  private getAIBrainState(botId: number): AIBrainState {
    if (!this.aiBrainStates.has(botId)) {
      this.aiBrainStates.set(botId, {
        regime: {
          regime: 'ranging',
          confidence: 0.5,
        ...
        metaLearning: {
          ...
          optimalConditions: {
            bestRsiEntry: 35,
            bestHoldTime: 4, // hours
            bestTimeOfDay: 14, // 2 PM UTC
            worstTimeOfDay: 3  // 3 AM UTC
          },
          learningRate: 0.1,
    ...
    return this.aiBrainStates.get(botId)!;
  }
  ...
  private async detectMarketRegime(
    ...
    const brainState = this.getAIBrainState(botId);
    ...
    // Calculate volatility percentile with edge case handling
    let volatilityPercentile = 50; // Default to median
    if (this.volatilityHistory.length > 0) {
      const sortedVol = [...this.volatilityHistory].sort((a, b) => a - b);
      const volIndex = sortedVol.findIndex(v => v >= volatility);
      // If volatility higher than all history, use 100th percentile; if not found use 100
      volatilityPercentile = volIndex === -1 
        ? 100 
        : Math.max(0, Math.min(100, (volIndex / sortedVol.length) * 100));
    }
    
    // Calculate trend strength using ADX-like logic
    const closes = ohlcv.slice(-20).map((c: any) => c[4]);
    let trendStrength = 0;
    if (closes.length >= 10) {
      const recentChanges = closes.slice(-10).map((c, i) => i > 0 ? c - closes[closes.length - 10 + i - 1] : 0);
      ...
      trendStrength = Math.abs(avgChange / avgPrice) * 1000; // Normalize to ~0-100
    }
    
    // Determine regime using multi-factor analysis
    let regime: MarketRegimeState['regime'] = 'ranging';
    let confidence = 0.5;
    
    const trendDirection = emaFast > emaSlow ? 'bullish' : 'bearish';
    const trendGap = Math.abs(emaFast - emaSlow) / emaSlow * 100;
    
    if (volatilityPercentile > 80) {
      regime = 'volatile';
      confidence = 0.7 + (volatilityPercentile - 80) / 100;
    } else if (trendGap > 2 && trendStrength > 30) {
      regime = trendDirection === 'bullish' ? 'trending_bullish' : 'trending_bearish';
      confidence = 0.6 + Math.min(0.3, trendGap / 10);
    } else if (rsi < 35 && volatilityPercentile < 40) {
      regime = 'accumulation';
      confidence = 0.55 + (35 - rsi) / 100;
    } else if (rsi > 65 && volatilityPercentile < 40) {
      regime = 'distribution';
    ...
    const regimeState: MarketRegimeState = {
      regime,
  ...
  private matchPattern(
    ...
    regime: string,
    volumeProfile: string
  ): { pattern: PatternMemory | null; matchScore: number } {
    let bestMatch: PatternMemory | null = null;
    let bestScore = 0;
    
    for (const pattern of this.patternDatabase) {
      let score = 0;
      
      // RSI range match (40% weight)
      if (rsi >= pattern.conditions.rsiRange[0] && rsi <= pattern.conditions.rsiRange[1]) {
        score += 0.4;
      } else {
        const distance = Math.min(
          Math.abs(rsi - pattern.conditions.rsiRange[0]),
          Math.abs(rsi - pattern.conditions.rsiRange[1])
        );
        score += Math.max(0, 0.4 - (distance / 50));
      }
      
      // Trend direction match (20% weight)
      if (pattern.conditions.trendDirection === trendDirection || pattern.conditions.trendDirection === 'neutral') {
        score += 0.2;
      }
      
      // Volatility level match (15% weight)
      if (pattern.conditions.volatilityLevel === volatilityLevel) {
        score += 0.15;
      }
      
      // Regime match (15% weight)
      if (pattern.conditions.regime === regime || pattern.conditions.regime.includes(regime)) {
        score += 0.15;
      }
      
      // Volume profile match (10% weight)
      if (pattern.conditions.volumeProfile === volumeProfile) {
        score += 0.1;
      }
    ...
    return { pattern: bestMatch, matchScore: bestScore };
  }
  
  // ============== SELF-REFLECTION: LEARN FROM MISTAKES ==============
  private async performSelfReflection(botId: number): Promise<{ 
    shouldAdjustConfidence: number; 
    insights: string[];
    warnings: string[];
  }> {
    const brainState = this.getAIBrainState(botId);
    const trades = await storage.getTrades(botId);
    const recentTrades = trades.filter(t => t.side === 'sell' && t.pnl !== null).slice(-10);
    ...
    if (recentTrades.length < 3) {
      return { shouldAdjustConfidence: 0, insights: ['Insufficient data for self-reflection'], warnings: [] };
    }
    ...
    // Pattern analysis from losses
    if (losses.length >= 3) {
      const consecutiveLosses = this.countConsecutive(recentTrades.map(t => (t.pnl || 0) > 0), false);
      if (consecutiveLosses >= 3) {
        confidenceAdjustment -= 0.15;
        brainState.selfReflection.recentMistakes.push(`${consecutiveLosses} consecutive losses detected - reducing confidence`);
        warnings.push('Losing streak detected - AI reducing position sizes');
      }
    }
    
    // Analyze time of day patterns
    const winsByHour = new Map<number, { wins: number; total: number }>();
    for (const trade of recentTrades) {
      const hour = new Date(trade.timestamp).getUTCHours();
      const stats = winsByHour.get(hour) || { wins: 0, total: 0 };
      stats.total++;
      if ((trade.pnl || 0) > 0) stats.wins++;
      winsByHour.set(hour, stats);
    }
    
    // Find best/worst hours
    let bestHour = -1, worstHour = -1, bestRate = 0, worstRate = 1;
    winsByHour.forEach((stats, hour) => {
      if (stats.total >= 2) {
    ...
    if (bestHour >= 0) {
      brainState.metaLearning.optimalConditions.bestTimeOfDay = bestHour;
      insights.push(`Best performance at ${bestHour}:00 UTC (${(bestRate * 100).toFixed(0)}% win rate)`);
    }
    if (worstHour >= 0) {
      brainState.metaLearning.optimalConditions.worstTimeOfDay = worstHour;
      warnings.push(`Avoid trading at ${worstHour}:00 UTC (${(worstRate * 100).toFixed(0)}% win rate)`);
    }
    
    // Win rate based adjustments
    if (winRate >= 0.7) {
      confidenceAdjustment += 0.1;
      insights.push('High win rate - AI increasing confidence');
      brainState.selfReflection.lessonsLearned.push('Current strategy working well');
    } else if (winRate <= 0.3) {
      confidenceAdjustment -= 0.2;
      warnings.push('Low win rate - AI entering cautious mode');
      brainState.selfReflection.lessonsLearned.push('Need to be more selective with entries');
    }
    
    brainState.selfReflection.confidenceAdjustment = confidenceAdjustment;
    
    return { shouldAdjustConfidence: confidenceAdjustment, insights, warnings };
  }
  
  // Count trailing consecutive occurrences of value (from end of array)
  private countConsecutive(arr: boolean[], value: boolean): number {
    let current = 0;
  ...
  private async optimizeStrategy(botId: number, currentConditions: {
    regime: string;
    volatility: number;
    rsi: number;
    sentiment: number;
    ...
    // Default thresholds
    let recommendedStrategy = bot?.strategyType || 'adaptive';
    let minConfidence = bot?.minAiConfidence || 0.6;
    let minEntryScore = bot?.minEntryScore || 4;
    let positionSizeMultiplier = 1.0;
    let reasoning = '';
    
    // Apply self-reflection adjustments
    const reflection = await this.performSelfReflection(botId);
    minConfidence = Math.max(0.4, Math.min(0.9, minConfidence + reflection.shouldAdjustConfidence));
    
    // Regime-based strategy selection
    switch (currentConditions.regime) {
      case 'trending_bullish':
      case 'trending_bearish':
        recommendedStrategy = 'trend_following';
        minConfidence *= 0.9; // Slightly lower threshold in trends
        positionSizeMultiplier = 1.2;
        reasoning = 'Strong trend detected - using trend following with larger positions';
        break;
        
      case 'volatile':
        recommendedStrategy = 'scalping';
        minConfidence *= 1.15; // Higher threshold in volatility
        positionSizeMultiplier = 0.7;
        minEntryScore = Math.min(7, minEntryScore + 1);
        reasoning = 'High volatility - using scalping with smaller positions and stricter entry';
        break;
        
      case 'ranging':
        recommendedStrategy = 'mean_reversion';
        minConfidence *= 1.0;
        reasoning = 'Ranging market - using mean reversion strategy';
        break;
        
      case 'accumulation':
        recommendedStrategy = 'adaptive';
        positionSizeMultiplier = 1.1;
        reasoning = 'Accumulation phase detected - potential reversal, moderate long bias';
        break;
        
      case 'distribution':
        recommendedStrategy = 'adaptive';
        positionSizeMultiplier = 0.8;
        reasoning = 'Distribution phase - reducing exposure, potential reversal down';
        break;
        
      default:
        recommendedStrategy = 'adaptive';
        reasoning = 'Mixed signals - using adaptive strategy';
    }
    
    // Time of day adjustment
    const currentHour = new Date().getUTCHours();
    if (currentHour === brainState.metaLearning.optimalConditions.worstTimeOfDay) {
      minConfidence *= 1.2;
      positionSizeMultiplier *= 0.5;
      reasoning += ' | Caution: Historically poor hour';
    } else if (currentHour === brainState.metaLearning.optimalConditions.bestTimeOfDay) {
      minConfidence *= 0.95;
      positionSizeMultiplier *= 1.1;
      reasoning += ' | Optimal trading hour';
    }
    
    return {
      recommendedStrategy,
      adjustedThresholds: {
        minConfidence: Math.max(0.4, Math.min(0.95, minConfidence)),
        minEntryScore: Math.max(2, Math.min(8, minEntryScore)),
  ...
  // ============== ENHANCED AI ANALYSIS WITH BRAIN STATE ==============
  private async getEnhancedAIDecision(
    botId: number,
    symbol: string,
    price: number,
    rsi: number,
    ...
    const regimeState = await this.detectMarketRegime(ohlcv, rsi, emaFast, emaSlow, volatility, botId);
    
    // 2. Match patterns
    const trendDirection = emaFast > emaSlow ? 'up' : emaFast < emaSlow ? 'down' : 'neutral';
    const volatilityLevel = regimeState.volatilityPercentile > 70 ? 'high' : regimeState.volatilityPercentile < 30 ? 'low' : 'medium';
    const volumeProfile = volume > avgVolume * 1.3 ? 'surge' : volume < avgVolume * 0.7 ? 'declining' : 'normal';
    
    const patternResult = this.matchPattern(rsi, trendDirection, volatilityLevel, regimeState.regime, volumeProfile);
    
    // 3. Get meta-learning optimizations
    ...
    // 4. Get base AI decision with enhanced context
    const baseDecision = await this.analyzeMarketWithAI(
      symbol, price, rsi, emaFast, emaSlow, volatility, volume, avgVolume, sentiment
    );
    
    // 5. Apply pattern-based confidence adjustment
    let adjustedConfidence = baseDecision.confidence;
    const metaInsights: string[] = [];
    
    if (patternResult.pattern && patternResult.matchScore > 0.7) {
      const patternBoost = patternResult.pattern.outcome.winRate > 0.6 ? 0.1 : 0;
      adjustedConfidence = Math.min(0.95, adjustedConfidence + patternBoost);
      metaInsights.push(`Pattern: ${patternResult.pattern.patternId} (${(patternResult.matchScore * 100).toFixed(0)}% match, ${(patternResult.pattern.outcome.winRate * 100).toFixed(0)}% historical WR)`);
    }
    
    // 6. Apply meta-learning confidence adjustment with safe division
    const bot = await storage.getBot(botId);
    const baseMinConfidence = bot?.minAiConfidence && bot.minAiConfidence > 0 ? bot.minAiConfidence : 0.6;
    const confidenceRatio = optimization.adjustedThresholds.minConfidence / baseMinConfidence;
    adjustedConfidence = Math.max(0.1, Math.min(0.95, adjustedConfidence * confidenceRatio));
    metaInsights.push(optimization.reasoning);
    
    // 7. Log enhanced analysis
    await storage.createLog({
      botId,
      level: 'info',
      message: `[AI-BRAIN] Regime: ${regimeState.regime} (${(regimeState.confidence * 100).toFixed(0)}%) | Pattern: ${patternResult.pattern?.patternId || 'none'} | Strategy: ${optimization.recommendedStrategy} | Conf: ${(adjustedConfidence * 100).toFixed(0)}%`
    });
    
    return {
      action: baseDecision.action,
      confidence: adjustedConfidence,
      reasoning: `${baseDecision.reasoning} | ${optimization.reasoning}`,
      regime: regimeState.regime,
      patternMatch: patternResult.pattern?.patternId || null,
      metaInsights
    };
  }
...
Identify the chart pattern:`
  ...
  private async processMultiCoinTrading(bot: Bot) {
    // CRITICAL: Set this.botId to ensure all helper functions use the correct bot ID
    // This is necessary because multiple bots may run concurrently
    this.botId = bot.id;
    ...
    try {
      ...
      if (openPosition) {
        ...
        if (currentPrice > highestPrice) {
          ...
          await storage.updateBot(bot.id, { highestPrice });
        }
        ...
        if (currentPrice < slPrice || currentPrice > tpPrice || momentumLoss || rsi > 78 || sentimentExit || aiEarlyExit || macdWeakening || macdBearCross || nearResistanceExit) {
          ...
          await storage.createTrade({
            botId: bot.id, symbol, side: 'sell', price: currentPrice,
          ...
          if (!bot.isLiveMode) {
            ...
            await storage.updateBot(bot.id, { paperBalance: newBalance });
          }
          
          await storage.updateBot(bot.id, { highestPrice: null }); 
        ...
        // Enhanced AI Decision Making with Adaptive Brain - uses regime detection, pattern matching, and meta-learning
        const enhancedAI = await this.getEnhancedAIDecision(
          bot.id, symbol, currentPrice, rsi, emaFast, emaSlow,
          volatilityScaling, currentVolume, avgVolume, sentiment, ohlcv
        );
        
        // Use enhanced AI analysis which includes regime, pattern matching, and self-reflection
        const aiAnalysis = {
          action: enhancedAI.action,
          confidence: enhancedAI.confidence,
          reasoning: enhancedAI.reasoning,
          riskLevel: 'medium' as const
        };
        
        // AI Pattern Recognition (legacy - now also handled by enhanced AI brain)
        const patternAnalysis = await this.detectPatternWithAI(closes, symbol);
        
        // Log enhanced AI reasoning for transparency
        await storage.createLog({
          botId: bot.id,
          level: 'info',
          message: `AI Analysis: ${aiAnalysis.action.toUpperCase()} (${(aiAnalysis.confidence * 100).toFixed(0)}%) | Regime: ${enhancedAI.regime} | Pattern: ${enhancedAI.patternMatch || 'none'}`
        });
        ...
        if (shouldEnter) {
          ...
          await storage.createTrade({
            botId: bot.id, symbol, side: 'buy', price: currentPrice,
      ...
      await storage.createLog({ botId: bot.id, level: "error", message: `Strategy Error: ${error.message}` });
    }
  }
